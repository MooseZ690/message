...
def removeadmin(id):
    if query_db("SELECT 1 FROM admins WHERE userid = ?", (id,)):
        # if user is admin
        db = get_db()
        db.execute(""" DELETE FROM admins WHERE userid = ?; """, (id,))
        db.commit()
    return redirect(request.referrer)
...
def like(id):
    ...
    db.execute("INSERT INTO likes (liker_id, postid) VALUES (?, ?)", (liker_id, id))
    db.commit()
...
def unlike(id):
    ...
    db.execute("DELETE FROM likes WHERE liker_id = ? AND postid = ?", (liker_id, id))
    db.commit()
...
def login():
    if request.method == "POST":
        ...
        user = query_db("SELECT * FROM users WHERE name = ?", (username,), one=True)
...
@app.route(
    "/newpost/<int:id>", methods=["GET", "POST"]
...
# flask app route for the page that shows posts only from a certain category
def category(id):
    if request.method == "POST":
        comment_text = request.form.get("comment")
        id = request.form.get("post_id")
...
def handle_send_message(data):
    ...
    emit(
        "receive_message",
        {"username": username, "message": message, "time": time},
        broadcast=True,
    )
